...
COLOUR_DIGITS = string.digits + string.ascii_lowercase
...
class XpmImage(object):
    ...
    def __init__(self, size, pixels, variable_name='picture', transparent_colour=None):
        ...
        for colour, char in zip(colours[:len(PRETTY_NAMES)], PRETTY_NAMES):
            self.colour_table[colour] = char * self.colour_width


    def make_header(self):
        return '\n'.join([
            b"/* XPM */",
            b"static char* {}[] = {{".format(self.variable_name),
            b""])
    ...
    def make_info(self):
        return [b'"{} {} {} {}"'.format(self.xsize, self.ysize, len(self.colours), 1)]
    ...
    def format_colour(self, colour):
        if colour == self.transparent_colour:
            ...
            return b'#{}{}{}'.format(*['{:02x}'.format(c) for c in colour])
...
if __name__ == '__main__':
    ...
    print(pil_save(image, variable_name=args.variable_name))
