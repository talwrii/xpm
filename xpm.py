...
COLOUR_DIGITS = (string.digits + string.ascii_lowercase).encode('utf8')
...
class XpmImage(object):
    ...
    def __init__(self, size, pixels, variable_name=b'picture', transparent_colour=None):
        ...
        colour_counts = collections.Counter(self.pixels[x, y] for x in range(self.xsize) for y in range(self.ysize))

        self.colours = [x for x, _ in sorted(colour_counts.items(), key=lambda x: -x[1])]
        self.colour_width = int(
            math.log(len(self.colours)) //
            math.log(len(COLOUR_DIGITS))) + 1
        self.colour_table = dict(zip(self.colours, enumerate_colours(self.colour_width)))
        ...
        for colour, char in zip(colours[:len(PRETTY_NAMES)], PRETTY_NAMES):

            self.colour_table[colour] = char.encode('ascii') * self.colour_width


    def make_header(self):
        return b'\n'.join([
            b"/* XPM */",
            "static char* {}[] = {{".format(self.variable_name.decode('ascii')).encode('ascii'),
            b""])
    ...
    def make_info(self):
        return ['"{} {} {} {}"'.format(self.xsize, self.ysize, len(self.colours), 1).encode('ascii')]
    ...
    def format_colour(self, colour):
        if colour == self.transparent_colour:
            ...
            return '#{}{}{}'.format(*['{:02x}'.format(c) for c in colour]).encode('ascii')
...
if __name__ == '__main__':
    ...
    PARSER = argparse.ArgumentParser(description='Turn an image into an XPM assuming PIL can open it')
    PARSER.add_argument('file', type=str, help='file to convert')
    PARSER.add_argument('--variable-name', '-v', type=str, help='XPM files are valid C code creating a variable representing an image. The name of this variable', default='picture')
    args = PARSER.parse_args()
    image = PIL.Image.open(args.file)
    xpm_bytes = pil_save(image, variable_name=args.variable_name.encode('ascii'))
